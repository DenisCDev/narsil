...
export class NexusError extends Error {
  ...
  constructor(message: string, opts: { code?: string; status?: number; details?: Record<string, unknown> } = {}) {
    super(message);
...
export class NexusPayloadTooLargeError extends NexusError {
  constructor(maxSize: number) {
    super(`Request body too large (max: ${maxSize} bytes)`, {
      code: "PAYLOAD_TOO_LARGE",
      status: 413,
      details: { maxSize },
    });
    this.name = "NexusPayloadTooLargeError";
  }
}
