...
export type RouteHandler = (ctx: any) => Promise<unknown>;
...
export class NexusRouter {
  /** Static routes: "GET:/api/users" → Route (O(1) lookup) */
   ...
   * Register a route.
   */
  add(method: HttpMethod, path: string, handler: RouteHandler, meta?: RouteMeta): void {
    const route: Route = { method, path, handler, module: meta?.module, operation: meta?.operation };
  ...
  private matchTrie(method: HttpMethod, path: string): { route: Route; params: Record<string, string> } | null {
    ...
    return this.matchTrieRecursive(this.root, segments, 0, method, params);
  }
